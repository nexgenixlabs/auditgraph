...
export function HumanIdentityRiskTable({ hiri }: HumanIdentityRiskTableProps) {
  const factors = [
    { key: 'h1_ghost', label: 'Ghost Humans', count: hiri?.h1_ghost ?? 0, color: COLORS.danger, nav: '/identity-explorer?tab=all&metric=ghost', tooltip: 'User accounts disabled or deprovisioned in the identity provider that still retain active role assignments. Ghost accounts can be exploited if re-enabled or if role assignments are inherited.', filter: 'last_login > 90d, account disabled' },
    { key: 'h2_dormant_priv', label: 'Dormant Privileged', count: hiri?.h2_dormant_priv ?? 0, color: COLORS.elevated, nav: '/identity-explorer?tab=all&metric=dormant_privileged', tooltip: 'Human users with privileged roles (Owner, Contributor, User Access Administrator) who have not signed in for 90+ days (default \u2014 configurable). Standing privilege without active use is a primary attack vector.', filter: 'privileged + inactive 30d+' },
    { key: 'h3_over_priv', label: 'Over-Privileged', count: hiri?.h3_over_priv ?? 0, color: COLORS.warning, nav: '/identity-explorer?tab=all&metric=over_permissioned', tooltip: 'Identities holding Tier-0 or Tier-1 role assignments with broad control or management access. Review whether this access level is operationally justified.', filter: 'privilege_tier 0-1' },
    { key: 'h4_ext_guest', label: 'Privileged Guests', count: hiri?.h4_ext_guest ?? 0, color: COLORS.purple, nav: '/identities?agirs_factor=h4_ext_guest', tooltip: 'External or guest identities who hold privileged roles such as Owner or Contributor. Guest access combined with high privilege significantly increases breach exposure.', filter: 'guest + privileged role' },
