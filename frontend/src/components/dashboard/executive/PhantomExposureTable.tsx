...
export function PhantomExposureTable({ nhiri }: PhantomExposureTableProps) {
  ...
  const categories: { key: string; label: string; count: number | null; color: string; nav: string; tooltip: string; filter: string }[] = [
    ...
    { key: 'dormant', label: 'Dormant NHIs', count: pb.dormant, color: COLORS.elevated, nav: '/identities?agirs_factor=n2_dormant', tooltip: 'Non-human identities with stale, inactive, or no observed activity status that still retain active role assignments. These represent latent attack surface.', filter: 'NHI + inactive 30d+' },
