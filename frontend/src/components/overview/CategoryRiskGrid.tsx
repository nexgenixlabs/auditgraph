...
import { IDENTITY_CATEGORIES } from '../../constants/metrics';
...
// Build config from shared constants + local icons
const categoryConfig: Record<string, { label: string; description: string; icon: React.ReactNode }> = Object.fromEntries(
  Object.entries(IDENTITY_CATEGORIES).map(([key, meta]) => [
    key,
    { label: meta.label, description: meta.description, icon: categoryIcons[key] || null },
  ])
);
